...
MemoryFileSystem.prototype.writeFileSync = function(_path, content, encoding) {
  // totally stub'ish!!!! lot's of trouble :)
  var watching = this.watching;
  this.hot = false;

  for(var j = 0; j < watching.length - 1; j++) {
      if(_path == watching[j]) {
          this.hot = true;
          break;
      }
...
// watch - totally stubbed!
MemoryFileSystem.prototype.watch = function(files, options) {
  this.watching.push(files);
}

MemoryFileSystem.prototype.hot = function() {
  return this.hot;
}
